...
adams({
  ...
  await repondre("*𝙱.𝙼.𝙱-𝚇𝙼𝙳 is sending your message to all groups ,,,💀*...");

  for (let groupId of groupIds) {
    let broadcastMessage = `*🌟B.M.B-XMD 𝐁𝐑𝐎𝐀𝐃𝐂𝐀𝐒𝐓🌟*\n\n🀄 Message: ${message}\n\n🗣️ Author: ${nomAuteurMessage}`;
...
adams({
  ...
  repondre(`A moment, *B.M.B-XMD* is compiling ${groupMetadata.participants.length} contacts into a vcf...`);
...
adams({
  ...
  if (!match) {
    return await client.sendMessage(msg, "Example: add 255772341432");
  }
      ...
      if (args[0] === 'on') {
        ...
        if (['remove', 'warn'].includes(action)) {
          if (action === 'remove') {
            ...
            respond("Antilink is now set to remove the link.");
          } else if (action === 'warn') {
            await ajouterOuMettreAJourJid(msg, 'warn');
            respond("Antilink is now set to warn the user.");
          }
        } else {
...
adams({
  ...
  if (!arg || arg.length === 0) {
    let muteStatus = groupCron && groupCron.mute_at ? 
      ...
      "No time set for automatic mute";
...
adams({
  ...
  const hentaiService = require('../bdd/hentai');
  ...
  try {
    if (!arg || !arg[0] || arg === " ") {
      repondre(`
        ...
        By default, the anti-word feature is set to delete.
      `);
      return;
    }
  ...
  try {
    if (!arg || !arg[0] || arg === " ") {
      repondre(`
        ...
        By default, the anti-link feature is set to delete.
      `);
      return;
    }
...
adams({
  ...
  const alldata = await mbdd.recupererToutesLesValeurs();
  ...
  if (!arg || arg.length < 1) {
    ...
    let msg = `
      ...
      *Instructions:*
      ...
      Example: mention https://static.animecorner.me/2023/08/op2.jpg image Hi, my name is adams.
